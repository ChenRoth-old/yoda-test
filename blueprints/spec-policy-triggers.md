...
The following policy triggers are defined in [`types.yaml`](hugo).
...
You can find the implementation for this trigger on [github](hugo). It builds the HTTP request to the manager REST service and makes the actual REST call using the `clj-http` clojure library.
