...
executor         | no       | string      | Valid values: `central_deployment_agent`, `host_agent`. See the [Plugins Specification](hugo) for more info.
...
For the sake of simplicity, we will not refer to [relationships](hugo) in these examples.
...
* Declared a [node type](hugo) with a `my_deployment_interface` interface that has a single `configure` operation which is mapped to the `deployer.config_in_master.configure` task.
...
Interface inputs are passed directly to a plugin's operation (as **kwargs to our `deploy` operation in the `deployer` plugin) or, in the case of our `start` operations, to the [Script Plugin](hugo).
hugo

# Relationship Interfaces

For information on relationship interfaces see [Relationships Specification](hugo#relationship-interfaces).
