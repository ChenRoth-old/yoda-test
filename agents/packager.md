...
Cloudify's Agent is basically a [virtualenv](hugo) with a series of modules installed in it and (optionally) a few configuration files attached.

To use Cloudify with distributions other than the [officially supported ones](hugo), we're providing an [Agent-Packager tool](https://github.com/cloudify-cosmo/cloudify-agent-packager) that will assist you in creating an agent for your distribution.
...
See the cloudify-agent [documentation](hugo#configuration) for more information.
...
- [Cloudify REST Client](hugo) (Mandatory)
- [Cloudify Plugins Common](hugo) (Mandatory)
...
- [Cloudify Script Plugin](hugo) (Optional)
- [Cloudify Diamond Plugin](hugo) (Optional)
...
the [script plugin](hugo) you'll have to explicitly configure it in the `additional_modules`
