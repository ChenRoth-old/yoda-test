...
To understand what a plugin represents, please refer to the plugins section in the [Architecture Overview](hugo).
...
`cloudify-plugins-common` documentation can be found [here](hugo).
...
If you want to unit test a specific function that needs a `ctx` object, you can use [`cloudify.mocks.MockCloudifyContext`](hugo) which is provided by `cloudify-plugins-common`.
...
(Note: `MockCloudifyContext` accepts various additional parameters. Check the [documentation](hugo) for more information.)

# The end (Sort of)

That's it! You just wrote your first plugin! All you need now is to incorporate it within your blueprint.
For additional info read the [Blueprint Guide](hugo).
...
See Cloudify's [OpenStack plugin](hugo) for reference.


# The Plugin Template

A Plugin Template is provided [here](hugo) to help you start writing your first plugin.
...
More info on the module can be found [here](hugo).
...
* The test_plugin.py file imports the `local` attribute from the cloudify.workflows module (a part of the `cloudify-plugins-common` module). This will allow you to run your operations locally using the [local workflows API](hugo).
...
* the `inputs` dictionary will allow you to supply [inputs](hugo) for your blueprint.
