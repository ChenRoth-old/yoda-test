...
The stream of events for each node instance is read from a deployment specific RabbitMQ queue by the policy engine ([Riemann](http://riemann.io/)). It may come from any source but is typically generated by a monitoring agent installed on the node's host. (See: [Diamond Plugin](hugo)).
...
Cloudify comes with a number of built-in policies, which are declared in [`types.yaml`](hugo) (which is usually imported either directly or indirectly via other imports).
...
We will show an example using the built-in [Diamond](hugo) documentation for further details on configuring diamond based monitoring.
