...
In this tutorial we will create a [blueprint](hugo#blueprint) that describes
the [topology](hugo#application).
We will write, step by step, a simplified version of the [Single Host Nodecellar](hugo) blueprint. <br>
...
- [Nodecellar](hugo) - This is the application business logic packed as a Node.JS application.
...
To learn more about blueprint versioning, refer to [Versions](hugo).
...
- [plugins](hugo#plugin)
- [node_types](hugo#type)
- [relationships](hugo#relationship-type)

For a complete reference of these constructs refer to the DSL Specification section.

In our case, we need to import Cloudify's [built-in types definitions](hugo) since our custom `node_types` will *derive from* some of these types.

**This will become clearer as we go along, don't worry :)**

To learn more about `imports`, please refer to [Imports Specification](hugo).
...
To learn more about `inputs`, please refer to [Inputs Specification](hugo).
...
To learn more about `node_types`, please refer to [Node Types Specification](hugo).
...
For more examples of using the *ctx* utility refer to [Context Proxy Utility](hugo). <br>
...
hugo
**All of the paths specified are relative to the blueprint file directory. You can find the scripts [here](hugo)**
...
To learn more about this, have a look at the full blown [Nodecellar example](hugo)
hugo
...
Cloudify's [built-in types definitions](hugo) comes with a relationship of type `cloudify.relationships.connected_to`, which seems to fit into our case.
...
Cloudify's [built-in types definitions](hugo) comes with a relationship of type `cloudify.relationships.contained_in`, which also seems appropriate.
...
To learn more about relationships, please refer to [Relationships Specification](hugo).
...
This [script](hugo#context-object)
to set [runtime properties](hugo#runtime-properties) that determine the MongoDB URL on the *source* node of this relationship.
...
To learn more about `node_templates`, please refer to [Node Templates Specification](hugo).
...
This type is one that is provided by Cloudify's [built-in types definitions](hugo).
...
- *install_agent* - This tells cloudify whether or not to install a Cloudify [Agent](hugo) on this host.
...
This is where [Intrinsic Functions](hugo) come in to play.
...
The `outputs` part of the blueprint is optional, but it is useful because `outputs` allow the blueprint to expose application characteristics via the [REST API](hugo). <br>
