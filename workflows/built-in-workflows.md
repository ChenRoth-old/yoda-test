...
Built-in workflows are declared and mapped in the blueprint in [`types.yaml`](hugo), which is usually imported either directly or indirectly via other imports.
...
The implementations for these workflows can be found at [`cloudify-plugins-common`](hugo).
